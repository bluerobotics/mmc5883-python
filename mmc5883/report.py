...
def generate_figures(log):
    ...
    footer = 'mmc5883 test report'

    f, spec = log.figure(height_ratios=[1, 1], suptitle='mmc5883 data', footer=footer)
    plt.subplot(spec[0,:])
    mag.stats().ttable(rl=True)

    plt.subplot(spec[1,:])
    mag.pplot(title='mmc5883 data')

    if getattr(log, 'error', False) is not False:
        f, spec = log.figure(height_ratios=[1], suptitle='mmc5883 errors', footer=footer)
...
def main():
    ...
    parser = LLogReader.create_default_parser(__file__, 'mmc5883')
